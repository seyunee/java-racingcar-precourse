...
public class Racing {
    ...
    public void start() {
        ...
        int turnCount = 0, tryCount = 0;

        tryCount = getTryCount();

        System.out.print("\n실행 결과");
        while (turnCount < tryCount) {
            turnCount++;
            Turn turn = new Turn();
            turn.turn(cars);
        }

        System.out.print("\n" + printWinners(getWinner(cars)) + "가 최종 우승했습니다.");
        scanner.close();
    }
    ...
    public int getTryCount() {
        ...
        try {
            return ValidationUtil.tryCount(userTryCount);
        } catch (IllegalArgumentException e) {
            System.out.println(e.getMessage());
            return getTryCount();
        }
